package cap8_EserciziRiepilogativi; //esercizio 8_JJ

//classe di test con un po' di metodi
public class TestPolilinea {
	private static final String TEST_OK = "TEST OK:\n";
	private static final String TEST_KO = "TEST KO:\n";
...
//implementiamo questi metodi con delle asserzioni, se il test fallisce deve essere lanciato un errore tramite asserzione
	...
	public static void testPolilineaConUnSoloSegmento() {
		try {
			...
			System.out.println(TEST_KO + pol1);
			assert false : "E' stata creata un polilinea con un solo segmento!";
		} catch (Exception exc) {
			System.out.println(TEST_OK + exc.getMessage());
			System.out.println();
		}
	...
	public static void testPolilineaConDueSegmentiNonConsecutiviNelCostruttore() {
		try {
			...
			Polilinea pol1 = new Polilinea(s1, s2);
			System.out.println(TEST_KO + pol1);
			assert false : "E' stata creata una polilinea con due segmenti " + " non consecutivi (nel costruttore)!";
		} catch (Exception exc) {
			System.out.println(TEST_OK + exc.getMessage());
			System.out.println();
		}
	...
	public static void testPolilineaConTreSegmentiNonConsecutiviNelCostruttore() {
		try {
			...
			System.out.println(TEST_KO + pol1);
			assert false : "E' stata creata una polilinea con tre segmenti " + "non consecutivi (nel costruttore)!";
		} catch (Exception exc) {
			System.out.println(TEST_OK + exc.getMessage());
			System.out.println();
		}
	...
	public static void testPolilineaScorrettaAggiungendoUnSegmentoNonConsecutivo() {
		try {
			...
			pol1.aggiungiSegmento(s3);
			System.out.println(TEST_KO + pol1);
			assert false : "È stata creata un polilinea con un segment" + " non consecutivo!";
		} catch (Exception exc) {
			System.out.println(TEST_OK + exc.getMessage());
		}
	}
