...
//astrae il concetto di linea spezzata
import java.util.*;

public class Polilinea {
	// deve essere un ArrayList di segmenti
...
//la polilinea ha il vincolo che il secondo estremo di un segmento deve coincidere col primo estremo del segmento
//successivo. Inoltre la polilinea deve avere ALMENO due segmenti.
	...
	private boolean controllaSegmentiConsecutivi() {
        ...
        for (int i = 0; i < segmentiListSize-1; ) {
            Punto secondoEstremoSegmentoPrecedente =
                segmentiList.get(i++).getEstremo2();
            Punto primoEstremoSegmentoSuccessivo =
                segmentiList.get(i).getEstremo1();
            if (!uguali(secondoEstremoSegmentoPrecedente,
                primoEstremoSegmentoSuccessivo)) {
                return false;
            }
        }
        return true;
